...
namespace SOR4_Blender
{
    public class SOR4Bot
    {
        ...
        public static Dictionary<string, int> characterIDs = new()
        {
            ["axel"] = 1,
            ["blaze"] = 2,
            ["cherry"] = 3,
        ...
        public List<string> enabledCharacters = new();

        public string RunCommand(string[] commandpair, string chatsender, List<int> allowedParams)
        {
            ...
            if (no_of_players > 0)
            {
                for (int player_no = 1; player_no <= no_of_players; player_no++)
                {
                    ...
                    if (botCommandsList.ContainsKey(command))
                    {
                        ...
                        string tempResponse = ExecuteCommand(chatsender, player_no, item, command, allowedParams: allowedParams, value: newValue, mode: mode);
        ...
        private string ExecuteCommand(string chatsender, int player_no, string item, string command, List<int> allowedParams, int? value = 0, string mode = "", int? addl_int = 0)
        {
            int newValue;
            ...
            if (mode != "")
            {
                ...
                {
                    case "random":
                        int randomValue = CoinFlip(player_no, item, allowedParams);
                        int internalValue;
                        ...
                        if (item == "moves")
                        {
                            for (int i = 0; i <= 4; i++)
                            {
                                internalValue = CoinFlip(player_no, item, allowedParams);
                                returnString = ExecuteCommand(chatsender, player_no, item, internalCommand, allowedParams, internalValue, "", i);
                            }
                        }
                        else
                        {
                            returnString = ExecuteCommand(chatsender, player_no, item, internalCommand, allowedParams, internalValue);
                        }
        ...
        {
            [0] = Enumerable.Range(0, 3).ToArray(),
            [1] = Enumerable.Range(3, 3).ToArray()
        };
        ...
        private int CoinFlip(int player_no, string item, List<int> allowedParams)
        {
            ...
            switch (item)
            {
                ...
                case "health":
                    ...
                    int temp_side = random_range[0];
                    if (allowedParams.Count() > 0)
                    {
                        while (allowedParams.IndexOf(temp_side) == -1)
                        {
                            random_range = bias[item][side].OrderBy(_ => randomgenerator.Next()).ToArray();
                            temp_side = random_range[0];
                        }
                    }
                    side = temp_side;
                    break;
                case "character":
                    int char_side = randomgenerator.Next(allowedParams.Count());
                    side = allowedParams[char_side];
                    break;
            }
            return side;
        }
